...
def test_each_dataset_declares_minimum_contract(dataset_configs: list[Path]) -> None:
    for cfg in dataset_configs:
        ...
        assert dataset.get("output", {}).get("artifacts") in {"minimal", "standard", "debug"}, f"{rel}: output.artifacts non valido"
...
def test_no_run_outputs_committed() -> None:
    ...
    result = subprocess.run(
        ["git", "ls-files", "--error-unmatch"] + [str(p) for p in OUT_DIR.rglob("*") if p.is_file()],
        capture_output=True, text=True, cwd=REPO_ROOT,
    )
    tracked = [l for l in result.stdout.strip().splitlines() if l]
    assert not tracked, f"Non committare output di run in out/: {tracked[:10]}"
